# Build the oblikovati-spacemouse add-in as a c-shared library (.so/.dll/.dylib) and
...
# version is independent of the host (pinned to match go.mod's floor; oblikovati.org/api
# v0.152.0+ requires go >= 1.27.0, see go.mod).
...
test: ## Run the pure-Go tests (camera math + bridge), no device/cgo
	CGO_ENABLED=0 go test -tags nodevice ./navigate/... ./bridge/... ./device/...
