...
// Command oblikovati-spacemouse is built as a c-shared library (.so/.dll/.dylib) and
...
// `make sync-header`): on Activate it constructs the navigation engine (package bridge)
// bound to the host through the host-supplied callback, opens the 3Dconnexion device,
// and drives the viewport camera from its 6-DOF motion. The host owns the camera; this
// library owns the device read loop and the camera math.
package main
...
import (
	...
	"oblikovati.org/api"
	"oblikovati.org/spacemouse/bridge"
...
var (
	...
	engine   *bridge.Engine // active engine, nil when inactive
)
...
func ObkAddInActivate(call C.ObkHostCall, freeFn C.ObkHostFree) C.int {
	...
	eng := bridge.NewEngine(cgoHostCaller{}, device.Open)
	engine = eng
	// Register commands and start the device loop OFF the session goroutine: Activate
	// runs on the host's session goroutine before the frame loop starts, and a host call
	// there blocks until the loop drains the dispatcher — so registering inline would
	// deadlock the head (same pattern as the CAM/MCP/FEMM bridges). The running frame loop
	// drains this goroutine's host calls.
	go func() { _ = eng.Setup() }()
	return C.OBK_OK
}
...
func ObkAddInDeactivate() C.int {
	...
	mu.Unlock()
	if eng != nil {
		eng.Stop() // close the device read loop cleanly
	}
	return C.OBK_OK
}
